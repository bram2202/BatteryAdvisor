...
public class HomeAssistantWebSocketResponseServiceTests
{
    ...
    public async Task ReceiveForMessageIdAsync_ParsesResultArrayToStaticIdModelArray()
    {
        ...
        Assert.Equal("sensor.dsmr_energy_consumed_tariff_1", result[0].StatisticId);
        Assert.Equal("kWh", result[0].DisplayUnitOfMeasurement);
        Assert.False(result[0].HasMean);
        Assert.Equal(0, result[0].MeanType);
        Assert.True(result[0].HasSum);
        Assert.Null(result[0].Name);
        Assert.Equal("recorder", result[0].Source);
        Assert.Equal("kWh", result[0].StatisticsUnitOfMeasurement);
        Assert.Equal("energy", result[0].UnitClass);

        Assert.Equal("sensor.dsmr_energy_consumed_tariff_2", result[1].StatisticId);
    }
